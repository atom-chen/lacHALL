...
if not superok then
    -- printf("VivoLogin super init failed :"..ImplClass)
    ImplClass=nil
end
local VivoLogin=class(ModuleName,ImplClass)

function VivoLogin:ctor(onlogincallback)
    self.onlogincallback_=onlogincallback
end

function VivoLogin:invokeCallback(errcode,...)
    if self.onlogincallback_ then
        self.onlogincallback_(errcode,...)
    end
end

function VivoLogin:callAuth(args, onauthcallback)
...
--调用登录
function VivoLogin:doLogin()
    GameApp:dispatchEvent(gg.Event.SHOW_LOADING, "正在拉起登录...", 3)
    self:callAuth({})
end

function VivoLogin:onAuthResp(args)
    ...
    if args.errcode == 0 then
        ...
        local data ={type = "vivo", authtoken=args.authtoken, username=args.username}
        gg.Dapi:loginByThirdParty(data, handler(self, self.onLoginResp))
    else
        GameApp:dispatchEvent(gg.Event.SHOW_LOADING)
        GameApp:dispatchEvent(gg.Event.SHOW_TOAST,"授权失败!")
        printf("VivoLogin onAuthResp errcode:%s ",tostring(args.errcode))
...
function VivoLogin:checkLoginData(data)
...
function VivoLogin:onLoginResp(data)
    printf("VivoLogin onLoginResp %s",json.encode(checktable(data)))
    ...
    local cookiedata = { id = userid, userfrom = USER_FROM_VIVO }
    if idx then
