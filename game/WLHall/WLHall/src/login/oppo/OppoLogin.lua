...
if not superok then
    -- printf("OppoLogin super init failed :"..ImplClass)
    ImplClass=nil
end
local OppoLogin=class(ModuleName,ImplClass)

function OppoLogin:ctor(onlogincallback)
    self.onlogincallback_=onlogincallback
end

function OppoLogin:invokeCallback(errcode,...)
    if self.onlogincallback_ then
        self.onlogincallback_(errcode,...)
    end
end

function OppoLogin:callAuth(args, onauthcallback)
...
--调用登录
function OppoLogin:doLogin()
    GameApp:dispatchEvent(gg.Event.SHOW_LOADING, "正在拉起登录...", 3)
    self:callAuth({})
end

function OppoLogin:onAuthResp(args)
    ...
    if args.errcode == 0 then
        ...
        local data ={type = "oppo", token=args.token, ssoid=args.ssoid}
        gg.Dapi:loginByThirdParty(data, handler(self, self.onLoginResp))
    else
        GameApp:dispatchEvent(gg.Event.SHOW_LOADING)
        GameApp:dispatchEvent(gg.Event.SHOW_TOAST,"授权失败!")
        printf("OppoLogin onAuthResp errcode:%s ",tostring(args.errcode))
...
function OppoLogin:checkLoginData(data)
...
function OppoLogin:onLoginResp(data)
    printf("OppoLogin onLoginResp %s",json.encode(checktable(data)))
    ...
    local cookiedata = { id = userid, userfrom = USER_FROM_OPPO }
    if idx then
