...
FILE_TYPE_INFO = {
    ...
    'sourcecode.c.h': 'PBXHeadersBuildPhase',
...
class XcodeProject(PBXDict):
    ...
    def get_build_phases(self, phase_name, target_name=None):
        phases = None
        if not target_name:
            ...
            if target:
                for buildPhaseID in target['buildPhases']:
                    buildPhase = self.objects[buildPhaseID]
                    if buildPhase.get('isa') == phase_name:
                        phases = buildPhase
                        break

        return phases

    def get_build_files_for_phases(self, phase_obj):
        ret = []
        if phase_obj:
            for fID in phase_obj['files']:
                ret.append(self.objects[fID])
    ...
    def get_target_name(self, type='app'):
        target_name = None
        check_type = 'com.apple.product-type.application'
        if type == 'unit-test':
            check_type = 'com.apple.product-type.bundle.unit-test'
        elif type == 'ui-test':
            check_type = 'com.apple.product-type.bundle.ui-testing'

        for obj in self.objects.values():
            if obj.get('isa') == 'PBXNativeTarget' and obj.get("productType") == check_type:
                target_name = obj.get('name')
                break

        return target_name

    def get_info_plist_path(self, target_name, mode='Release'):
        ret = None
        try:
            target_obj = self.get_native_target(target_name)

            # get build configuration list
            list_id = target_obj.get('buildConfigurationList')
            list_obj = self.get_obj(list_id)

            # get the configuration
            for cfg_id in list_obj.get('buildConfigurations'):
                cfg_obj = self.get_obj(cfg_id)
                if cfg_obj.get('name') == mode:
                    settings = cfg_obj.get('buildSettings')
                    plist_cfg = settings['INFOPLIST_FILE']
                    if plist_cfg is not None:
                        ret = os.path.abspath(os.path.join(self.source_root, plist_cfg))
                        break
        except:
            pass
    ...
    def set_build_file_settings(self, build_file, settings):
        if not build_file:
            return

        build_file['settings'] = PBXDict()
        for k in settings:
            build_file['settings'][k] = settings[k]
    ...
    def set_target_service_enabled(self, target, service_name, enabled):
        target_obj = self.get_native_target(target)
        project_obj = self.get_native_target('PROJECT')
        target_attr = project_obj.get('attributes').get('TargetAttributes').get(target_obj.id)
        capabilities = target_attr.get('SystemCapabilities')
        if capabilities is None:
            capabilities = PBXDict()
            target_attr['SystemCapabilities'] = capabilities

        service = capabilities.get(service_name)
        if service is None:
            service = PBXDict()
            capabilities[service_name] = service

        service['enabled'] = ('1' if enabled else '0')
        self.modified = True

    def set_target_attr(self, target, key, value):
        target_obj = self.get_native_target(target)
        project_obj = self.get_native_target('PROJECT')
        target_attr = project_obj.get('attributes').get('TargetAttributes').get(target_obj.id)
    ...
    def set_target_build_setting(self, target, key, value, cfg_name=None):
        target_obj = self.get_native_target(target)
        cfg_list_obj = self.get_obj(target_obj.get('buildConfigurationList'))
        cfg_ids = cfg_list_obj.get('buildConfigurations')
        for cfg_id in cfg_ids:
            cfg_obj = self.get_obj(cfg_id)
            if (cfg_name is not None) and (cfg_name != cfg_obj.get('name')):
                continue

            cur_settings = cfg_obj.get('buildSettings', PBXDict())
    ...
    def get_comment(self, key, whole_data):
        ...
        if obj_isa == "PBXBuildFile":
            ...
            elif fileType == "PBXHeadersBuildPhase":
                ret = "%s in %s" % (fileName, "Headers")
            else:
                ret = "%s" % fileName
