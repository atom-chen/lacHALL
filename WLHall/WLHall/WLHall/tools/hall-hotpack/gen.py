...
if __name__ == "__main__":
    ...
    parser.add_argument("--min-ver", dest="min_ver", help="Specify min version, default value is 1")

    (args, unknown) = parser.parse_known_args()

    min_ver = int(args.min_ver) if args.min_ver else 1
    ...
    cmd = '%s -p %s --min-ver %d --out-name %s --exclude %s --ver-ext-cfg %s' % (TOOLS_PATH, ufd_file_path, min_ver, args.version, EXCLUDE_FILE, DO_FILES_CFG)
    hall_folders = []
