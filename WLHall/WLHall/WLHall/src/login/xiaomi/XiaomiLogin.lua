...
if not superok then
    -- printf("XiaomiLogin super init failed :"..ImplClass)
    ImplClass=nil
end
local XiaomiLogin=class(ModuleName,ImplClass)

function XiaomiLogin:ctor(onlogincallback)
    self.onlogincallback_=onlogincallback
end

function XiaomiLogin:invokeCallback(errcode,...)
    if self.onlogincallback_ then
        self.onlogincallback_(errcode,...)
    end
end

function XiaomiLogin:callAuth(args, onauthcallback)
...
--调用登录
function XiaomiLogin:doLogin()
    GameApp:dispatchEvent(gg.Event.SHOW_LOADING, "正在拉起小米登录...", 3)
    self:callAuth({})
end

function XiaomiLogin:onAuthResp(args)
    printf("onAuthResp args " .. json.encode(checktable(args)))
    ...
    if args.errcode == 0 then
        ...
        local data ={type=args.from, uid=args.uid, nickname=args.nickname, session=args.session}
        gg.Dapi:loginByThirdParty(data, handler(self, self.onLoginResp))
    else
        GameApp:dispatchEvent(gg.Event.SHOW_LOADING)
        GameApp:dispatchEvent(gg.Event.SHOW_TOAST,"授权失败!")
        printf("XiaomiLogin onAuthResp errcode:%s ",tostring(args.errcode))
...
function XiaomiLogin:checkLoginData(data)
...
function XiaomiLogin:onLoginResp(data)
    printf("XiaomiLogin onLoginResp %s",json.encode(checktable(data)))
    ...
    local cookiedata = { id = userid, userfrom = USER_FROM_XIAOMI }
    if idx then
