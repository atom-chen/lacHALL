...
if not superok then
    -- printf("ToutiaoLogin super init failed :"..ImplClass)
    ImplClass=nil
end
local ToutiaoLogin=class(ModuleName,ImplClass)

function ToutiaoLogin:ctor(onlogincallback)
    self.onlogincallback_= onlogincallback
end

function ToutiaoLogin:invokeCallback(errcode,...)
    if self.onlogincallback_ then
        print("toutiao login callback ======errcode ===="..errcode)
...
function ToutiaoLogin:callAuth(args, onauthcallback)
...
--调用登录
function ToutiaoLogin:doLogin()
    GameApp:dispatchEvent(gg.Event.SHOW_LOADING, "正在拉起登录...", 3)
    self:callAuth()
end

function ToutiaoLogin:doLogout()
...
function ToutiaoLogin:onAuthResp(args)
    ...
    if args.errcode == 0 then
        ...
        local data ={type = "toutiao", token = args.token, uid = args.uid}
        gg.Dapi:loginByThirdParty(data, handler(self, self.onLoginResp))
    else
        GameApp:dispatchEvent(gg.Event.SHOW_LOADING)
        GameApp:dispatchEvent(gg.Event.SHOW_TOAST,"授权失败!")
        printf("ToutiaoLogin onAuthResp errcode:%s ",tostring(args.errcode))
...
function ToutiaoLogin:checkLoginData(data)
...
function ToutiaoLogin:onLoginResp(data)
    printf("ToutiaoLogin onLoginResp %s",json.encode(checktable(data)))
    ...
    local cookiedata = { id = userid, userfrom = USER_FROM_TOUTIAO }
    if idx then
