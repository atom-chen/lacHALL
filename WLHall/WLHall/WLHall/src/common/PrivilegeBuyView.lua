...
-- Author: zhaoxinyu
-- Date: 2017-02-19 17:45:20
-- Describe：贵族特权购买界面

local PrivilegeBuyView = class("PrivilegeBuyView", cc.load("ViewPop"))

PrivilegeBuyView.RESOURCE_FILENAME="ui/store/privilege_buy_view.lua"

PrivilegeBuyView.RESOURCE_BINDING = {
	["panel"]   = { ["varname"] = "panel" },
...
function PrivilegeBuyView:onCreate()
	self.goodid = "month_card_normal"
	-- 获取商品的支付方式
    ...
    -- 购买月卡成功的事件回调
    self:addEventListener(gg.Event.BUG_MONTHCARD_SUCCESS, handler(self, self.callbackBuySuccess) )
end

function PrivilegeBuyView:init()
...
function PrivilegeBuyView:callbackBuySuccess()
...
function PrivilegeBuyView:initView()

	-- 获取礼包配置
	local storeData = require("hall.models.StoreData")
	local privilegeGiftTable = storeData:GetPrivilegeGiftTable()

	-- 获取礼包价格
	local privilegeGoods = storeData:GetPrivilegeGoodsInfo()
...
function PrivilegeBuyView:setMoney( money , origPrice  )

	self.txt_price:setString( money )
	self.txt_ori_price:setString( origPrice )
end

function PrivilegeBuyView:setPrivilegeAdditional( leitai_ratio )

	local leitai_add = leitai_ratio * 100
	self.txt_privilege_brief:setString("贵族特权 :\n荣誉分 +"..leitai_add.."%")
end
...
function PrivilegeBuyView:setProps( props )
	if not props then
		return
	end
...
-- 根据豆豆数获取豆豆图片
function PrivilegeBuyView:getDouDouFile(count)
...
function PrivilegeBuyView:setDailyProps( props )
	if not props then
		return
	end
...
function PrivilegeBuyView:onClickClose( sender )
	self:removeSelf()
end
...
function PrivilegeBuyView:initPay( pay_methods )
...
function PrivilegeBuyView:onClickPay( sender )
	-- 播放点击音效
